...
def nested_simulation(N, edge_list, r_filt, p_filt, percent_of_stubborn_users, verified_users, seed):
    ...
    simulator.set_receiving_filter(r_filt)
...
for run in np.arange(runs)+1:
    ...
    if not os.path.exists(os.path.join(in_path, f'proper_polarizing_extremists_0-5_{num_points}-data_points_run_{run}.pickle')):
        # Load network
        ...
        with open(os.path.join(in_path, f'proper_polarizing_extremists_0-5_{num_points}-data_points_run_{run}.pickle'),'wb') as pickled_data_file:
...
for run in np.arange(5)+1:
    with open(os.path.join(in_path, f'proper_polarizing_extremists_0-5_{num_points}-data_points_run_{run}.pickle'),'rb') as pickled_data:
...
# ax = plt.gca()
# plt.plot(x_ax/100,mean_bc, color = '#8856a7')
# plt.fill_between(x_ax/100, shade_lower_limit, shade_upper_limit, alpha = 0.4, color = '#8856a7', linewidth=0)
# ax.axhline(5/9, linestyle='--', color='#404040')
# if mode == 'echo_chamber':
#     plt.ylabel(r'$BC_{\text{hom}}(\hat{b},\hat{b}_{NN})$')
# else:
#     plt.ylabel(r'$BC(\hat{b})$')
# plt.xlabel(r'fraction of ideologues $i$')
# plt.ylim(0,1)
# plt.xlim(0,0.05)
...
# if mode == 'echo_chamber':
#     plt.savefig(os.getcwd() + '/echo_chambers-by-extremists_10_verif_high-phi_' + str(num_points) + '-points_moments.pdf',bbox_inches='tight')
# else:
#     plt.savefig(os.getcwd() + '/polarization-by-extremists_10_verif_high-phi_' + str(num_points) + '-points_moments.pdf',bbox_inches='tight')
# plt.close()
# plt.clf()


# Plot 2 - median, 1st and 3rd quartiles
median_bc = np.array([el[1] for el in quantiles_bc])
shade_lower_limit = np.array([el[0] for el in quantiles_bc])
shade_upper_limit = np.array([el[2] for el in quantiles_bc])

ax = plt.gca()
plt.plot(x_ax/100,median_bc, color = '#8856a7')
plt.fill_between(x_ax/100, shade_lower_limit, shade_upper_limit, alpha = 0.4, color = '#8856a7', linewidth=0)
ax.axhline(5/9, linestyle='--', color='#404040')
if mode == 'echo_chamber':
    plt.ylabel(r'$BC_{\text{hom}}(\hat{b},\hat{b}_{NN})$')
...
if mode == 'echo_chamber':
    plt.savefig(os.path.join(out_path, 'figS4b.pdf'), bbox_inches='tight')
else:
    plt.savefig(os.getcwd() + '/polarization-by-extremists_10_verif_high-phi_' + str(num_points) + '-points_quantiles.pdf',bbox_inches='tight')
plt.close()
plt.clf()
...
# x_axis = np.array([])
# for x in x_ax:
#     x_axis = np.concatenate((x_axis,np.full(len(all_bc_values[0]),x/100)))
# y_axis = np.array([])
...
# #plt.title('BC' + r'(b)$=$' + str(bimodality_index(distr)))
# if mode == 'echo_chamber':
#     plt.savefig(os.getcwd() + '/echo_chambers-by-extremists_10_verif_high-phi_' + str(num_points) + '-points_all_vals.pdf',bbox_inches='tight')
# else:
#     plt.savefig(os.getcwd() + '/polarization-by-extremists_10_verif_high-phi_' + str(num_points) + '-points_all_vals.pdf',bbox_inches='tight')
# plt.close()
# plt.clf()
...
# #plt.title('BC' + r'(b)$=$' + str(bimodality_index(distr)))
# plt.savefig(os.getcwd() + '/echo_chambers-by-extremists_10_verif_high-phi_' + str(num_points) + '-points_peaks_alpha-for-height.pdf',bbox_inches='tight')
# plt.close()
# plt.clf()

## Plot 5 - Two peaks, alpha for mass

critical_y_min, _, _, critical_y_max, _, _ = peaks_of_distr(all_bc_values[19],5/9,100)
...
ax.axvline(0.019, critical_y_min, critical_y_max, linestyle='--', color='#8856a7', alpha=0.5) # 
ax.text(0.016, 0.03, r'$i_1$', horizontalalignment='center') # i = 1.1
ax.text(0.0205, 0.5, r'$i_c$', horizontalalignment='center') # i = 1.4
ax.text(0.022, 0.89, r'$i_2$', horizontalalignment='center') # i = 1.8
if mode == 'echo_chamber':
    plt.ylabel(r'$BC_{\text{hom}}(\hat{b},\hat{b}_{NN})$')
else:
    plt.ylabel(r'$BC(\hat{b})$')
...
if mode == 'echo_chamber':
    plt.savefig(os.path.join(out_path, 'fig6c.pdf'), bbox_inches='tight')
else:
    plt.savefig(os.getcwd() + '/polarization-by-extremists_10_verif_high-phi_' + str(num_points) + '-points_peaks_alpha-for-mass.pdf',bbox_inches='tight')
plt.close()
plt.clf()
...
y1_distr = all_bc_values[16]
kde1 = gaussian_kde(y1_distr, bw_method=0.1)
y1_ax = kde1.evaluate(x_axis)

y2_distr = all_bc_values[19]
kde2 = gaussian_kde(y2_distr, bw_method=0.1)
y2_ax = kde2.evaluate(x_axis)

y3_distr = all_bc_values[22]
kde3 = gaussian_kde(y3_distr, bw_method=0.1)
y3_ax = kde3.evaluate(x_axis)

plt.plot(x_axis,y1_ax, color = '#A1DAB4', label='$i_1=0.016$')
plt.plot(x_axis,y2_ax, color = '#41B6C4', label='$i_c=0.019$')
plt.plot(x_axis,y3_ax, color = '#225EA8', label='$i_2=0.022$')
...
if mode == 'echo_chamber':
    plt.savefig(os.path.join(out_path, 'fig6d.pdf'),bbox_inches='tight')
else:
    plt.savefig(os.getcwd() + '/polarization-by-extremists_10_verif_high-phi_' + str(num_points) + '-peak_distributions.pdf',bbox_inches='tight')
plt.close()
plt.clf()
