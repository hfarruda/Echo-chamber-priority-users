...
for network_type in ['er', 'power']:
    ...
    bc_homs = []
    if network_type == 'er' and os.path.exists(os.path.join(in_path, 'bc_hom_er_fig5.pickle')):
        with open(os.path.join(in_path, 'bc_hom_er_fig5.pickle'), 'rb') as handle:
            bc_homs = pickle.load(handle)
    elif network_type == 'power' and os.path.exists(os.path.join(in_path, 'bc_hom_power_fig5.pickle')):
        with open(os.path.join(in_path, 'bc_hom_power_fig5.pickle'), 'rb') as handle:
            bc_homs = pickle.load(handle)
    else:
        for i in tqdm(range(100)):
            ...
            for percentage in percentages:
                ...
                for percentage_stubborn in small_percentages:
                    ...
                    posting_filter[positions_aux] = doces.HALF_COSINE
        ...
        #save pickle here
        if network_type == 'er':
            with open(os.path.join(in_path, 'bc_hom_er_fig5.pickle'), 'wb') as handle:
                pickle.dump(bc_homs, handle)
        elif network_type == 'power':
            with open(os.path.join(in_path, 'bc_hom_power_fig5.pickle'), 'wb') as handle:
                pickle.dump(bc_homs, handle)
    ...
    ax.set_xlabel(r"Fraction of ideologues $i$")
    ax.set_ylabel(r"Fraction of priority users $r$")

    plt.tight_layout()

    if network_type == 'er':
        plt.savefig(os.path.join(out_path, 'fig5a.pdf'))
    elif network_type == 'power':
        plt.savefig(os.path.join(out_path, 'fig5b.pdf'))
    plt.close()